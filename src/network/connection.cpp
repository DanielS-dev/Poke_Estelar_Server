...
void Connection::closeSocket()
{
	if (socket.is_open()) {
		try {
			...
			LOG_ERROR("Network", std::string("Connection::closeSocket failed: ") + e.what());
		}
	}
}
...
void Connection::accept()
{
	...
	try {
		...
		LOG_ERROR("Network", std::string("Connection::accept failed: ") + e.what());
		close(FORCE_CLOSE);
	}
}
...
void Connection::parseHeader(const boost::system::error_code& error)
{
	...
	if ((++packetsSent / timePassed) > static_cast<uint32_t>(g_config.getNumber(ConfigManager::MAX_PACKETS_PER_SECOND))) {
		LOG_WARN("Network", convertIPToString(getIP()) + " disconnected for exceeding packet per second limit.");
		close();
		return;
	}
	...
	try {
		...
		LOG_ERROR("Network", std::string("Connection::parseHeader failed: ") + e.what());
		close(FORCE_CLOSE);
	}
}
...
void Connection::parsePacket(const boost::system::error_code& error)
{
	...
	try {
		...
		LOG_ERROR("Network", std::string("Connection::parsePacket failed: ") + e.what());
		close(FORCE_CLOSE);
	}
}
...
void Connection::internalSend(const OutputMessage_ptr& msg)
{
	...
	try {
		...
		LOG_ERROR("Network", std::string("Connection::internalSend failed: ") + e.what());
		close(FORCE_CLOSE);
	}
}
