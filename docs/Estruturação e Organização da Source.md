...
O padrão atual para logs técnicos é usar o logger central em `src/core/logger.cpp/.hpp`, com macros como `LOG_INFO`, `LOG_WARN`, `LOG_ERROR` e `LOG_FATAL`. Ele escreve no console e em arquivos separados por nível, com rotação configurável pelo `.env`.
